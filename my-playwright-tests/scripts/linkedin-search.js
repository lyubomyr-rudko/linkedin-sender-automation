...
const SEARCH_QUERY = process.env.LINKEDIN_SEARCH_QUERY;
...
const OUTPUT_FILE = process.env.LINKEDIN_OUTPUT_FILE
  ? path.resolve(process.env.LINKEDIN_OUTPUT_FILE)
  : path.join(RESULTS_DIR, 'linkedin-results.csv');
...
async function scrapeCurrentPage(page, maxToProcess = Infinity, seenProfiles = new Set()) {
  const pageResults = [];
...
async function scrapeResults(page, maxResults, seenProfiles = new Set()) {
  ...
  while (results.length < maxResults) {
    ...
    const pageResults = await scrapeCurrentPage(page, maxResults - results.length, seenProfiles);
    ...
    console.log(pageResults.length === 0 ? '⏭️  No new results on this page, moving on...' : '⏭️  Going to next page...');
...
function saveRunSnapshot(results, filename) {
  ...
  console.log(`🆕 Saved snapshot of this run to ${filename}`);
}
...
(async () => {
  ...
  try {
    ...
    if (existingFiles.length > 0) {
      console.log(`📁 Loaded ${existingProfiles.size} profiles from ${existingFiles.length} existing file${existingFiles.length > 1 ? 's' : ''}`);
    }
    const results = await scrapeResults(page, MAX_RESULTS, existingProfiles);
